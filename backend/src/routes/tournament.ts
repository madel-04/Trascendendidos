...
async function ensureNextRound(tournamentId: number): Promise<void> {
  ...
  const currentMatches = await query<MatchRow>(
    `SELECT id, tournament_id, round, match_order, player1_id, player2_id, winner_id, status, completed_at
  ...
  if (currentMatches.some((m) => m.status !== "completed" || !m.winner_id)) return;
  ...
  for (let i = 0; i < pairings.length; i += 1) {
    ...
    await query(
      `INSERT INTO tournament_matches (tournament_id, round, match_order, player1_id, player2_id)
       VALUES ($1, $2, $3, $4, $5)`,
      [tournamentId, nextRound, i + 1, player1, player2]
    );
  }
}
...
export async function tournamentRoutes(app: FastifyInstance) {
  app.get("/api/tournament", { onRequest: [app.authenticate] }, async (_request, reply) => {
    ...
    >(
      `SELECT t.id, t.name, t.creator_id, t.status, t.max_players, t.champion_id, t.created_at, t.started_at, t.ended_at,
    ...
    return reply.send({
      tournaments: tournaments.map((t) => ({
        ...
        creator: { id: t.creator_id, username: t.creator_username },
  ...
  app.post("/api/tournament", { onRequest: [app.authenticate] }, async (request, reply) => {
    try {
      ...
      const [created] = await query<TournamentRow>(
        `INSERT INTO tournaments (name, creator_id, max_players)
         VALUES ($1, $2, $3)
         RETURNING id, name, creator_id, status, max_players, champion_id, created_at, started_at, ended_at`,
        [body.name, userId, body.maxPlayers]
      );
  ...
  app.get<{ Params: { tournamentId: string } }>("/api/tournament/:tournamentId", { onRequest: [app.authenticate] }, async (request, reply) => {
    try {
      ...
      >(
        `SELECT tm.id, tm.tournament_id, tm.round, tm.match_order, tm.player1_id, tm.player2_id, tm.winner_id, tm.status, tm.completed_at,
                p1.username AS player1_username,
                p2.username AS player2_username,
                w.username AS winner_username
         FROM tournament_matches tm
         JOIN users p1 ON p1.id = tm.player1_id
         JOIN users p2 ON p2.id = tm.player2_id
      ...
      return reply.send({
        tournament: {
          ...
          creator: { id: tournament.creator_id, username: tournament.creator_username },
  ...
  app.post<{ Params: { tournamentId: string } }>("/api/tournament/:tournamentId/join", { onRequest: [app.authenticate] }, async (request, reply) => {
    try {
      ...
      const [tournament] = await query<TournamentRow>(
        `SELECT id, name, creator_id, status, max_players, champion_id, created_at, started_at, ended_at
         FROM tournaments
         WHERE id = $1
         LIMIT 1`,
        [tournamentId]
  ...
  app.post<{ Params: { tournamentId: string } }>("/api/tournament/:tournamentId/start", { onRequest: [app.authenticate] }, async (request, reply) => {
    try {
      ...
      if (participants.length !== tournament.max_players) {
        return reply.code(409).send({ error: `Se requieren ${tournament.max_players} jugadores para iniciar` });
      }
      ...
      for (let i = 0; i < pairings.length; i += 1) {
        ...
        await query(
          `INSERT INTO tournament_matches (tournament_id, round, match_order, player1_id, player2_id)
           VALUES ($1, 1, $2, $3, $4)`,
          [tournamentId, i + 1, player1, player2]
        );
      }
  ...
  app.post<{ Params: { tournamentId: string; matchId: string } }>("/api/tournament/:tournamentId/matches/:matchId/report", { onRequest: [app.authenticate] }, async (request, reply) => {
    try {
      ...
      await ensureNextRound(tournamentId);
